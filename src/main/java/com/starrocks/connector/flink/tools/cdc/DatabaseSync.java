...
import com.starrocks.connector.flink.tools.cdc.json.JsonFilter;
import com.starrocks.connector.flink.tools.cdc.json.JsonProcess;
import com.starrocks.connector.flink.tools.cdc.json.JsonSerializer;
import com.starrocks.connector.flink.catalog.starrocks.TableSchema;
import com.starrocks.connector.flink.tools.cdc.mysql.ParsingProcessFunction;
...
public abstract class DatabaseSync {
    ...
    public void build() throws Exception {
        ...
        for (SourceSchema schema : schemaList) {
            ...
            if (!starRocksCatalog.tableExists(database, starRocksTable)) {
                ...
                starRocksCatalog.createTable(starRocksSchema);
            }
        ...
        for (String table : starRocksTables) {
            ...
            sideOutput.map(new JsonProcess(getSerializer(table))).filter(new JsonFilter()).addSink(starRockSink).setParallelism(sinkParallel).name(table);
        }

    }

    private JsonSerializer getSerializer(String table) {
        String fenodes = sinkConfig.getString(StarRocksSinkOptions.FENODES);
        ...
        return JsonSerializer.builder().setStarRocksOptions(starRocksBuilder.build()).build();
    }
    ...
    private StarRocksSinkOptions getStarRocksSinkOptions(String table) {
        String jdbcUrl = sinkConfig.get(StarRocksSinkOptions.JDBC_URL);
        String loadUrl = sinkConfig.getString(StarRocksSinkOptions.FENODES).replace(",", ";");
        String user = sinkConfig.getString(StarRocksSinkOptions.USERNAME);
        String passwd = sinkConfig.getString(StarRocksSinkOptions.PASSWORD, "");
    ...
    private StarRocksJdbcConnectionOptions getStarRocksConnectionOptions() {
        ...
        return new StarRocksJdbcConnectionOptions(fenodes, user, passwd, jdbcUrl);
    }
